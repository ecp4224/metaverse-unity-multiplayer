...
namespace WalletConnectSharp.NEthereum.Account
{
    public class WalletConnectTransactionManager : TransactionManager
    {
        ...
        {
            var chainId = new HexBigInteger(new BigInteger(_session.ChainId));
            ...
            catch (WalletException e)
            {
                ...
                if (!SupportsEIP1559)
                {
                    signedTx = new LegacyTransactionChainId(transaction.To, transaction.Value,
                        transaction.Nonce, transaction.GasPrice, transaction.Gas, transaction.Data, chainId);
                    hash = "0x" + signedTx.RawHash.ToHex();
                }
                else
                {
                    signedTx = new Transaction1559(chainId, transaction.Nonce,
                ...
                if (!SupportsEIP1559)
                {
                    ...
                    //Calculate V
                    var v = chainId * new BigInteger(2) + recId + 35;
